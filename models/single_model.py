...
class SingleModel(BaseModel):
    ...
    def get_current_errors(self, epoch):
        D_A = self.loss_D_A.data.data
        D_P = self.loss_D_P.data.data if self.opt.patchD else 0
        G_A = self.loss_G_A.data.data
        if self.opt.vgg > 0:
            vgg = self.loss_vgg_b.data.data/self.opt.vgg if self.opt.vgg > 0 else 0
            return OrderedDict([('D_A', D_A), ('G_A', G_A), ("vgg", vgg), ("D_P", D_P)])
        elif self.opt.fcn > 0:
            fcn = self.loss_fcn_b.data.data/self.opt.fcn if self.opt.fcn > 0 else 0
